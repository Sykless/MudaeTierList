import { POOL_ID, proxifyImageUrl } from "../utils/Shared"
...
function Import({ mudaeImport, backupImport, triggerAnimation, animate }: ImportProperties) {
    ...
    function handleMudaeImport() {
        ...
        mudaeImport(importedCharacters)
        setImportText("")
        ...
        if (file.name.endsWith(".json") || file.name.endsWith(".png")) {
            reader.onload = async () =>
            {
                try {
                    ...
                    backupImport(backupData.tiers, backupData.pool);
                    toast.success("Données importées")
                }
                catch (e) {
